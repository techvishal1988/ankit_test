        ...
        domtoimage.toPng(node[0])
            .then(function (dataUrl) {
                var doc = new jsPDF('p', 'in', [w, h]),
                    text = "Performance Rating Dashobard"
                doc.addImage(dataUrl, 'JPEG', 0.13, 0.01, w, h);
                doc.save('performance-rating-dashboard.pdf')
                $('#loading').hide()
            });
    }, 100)
}
		...
		setTimeout(function() {
            ...
            domtoimage.toPng(node[0])
                ...
                if(i == chartArray.length) {
                    pptx.save('performance-rating-dashboard');
                    $('#loading').hide()
                }
            });
            }, 1000 * index);
...
function exportCsv(data) {
    ...
    'company_joining_date', 'rating_for_current_year'];
    var dataObj = {};
    var dataObjNew = [];
   ...
   $.each(data, function (key, val) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Performance Rating Dashobard');
    XLSX.writeFile(wb, "performance-rating-dashboard.xlsx");
}
