        ...
        domtoimage.toPng(node[0])
            .then(function (dataUrl) {
                var doc = new jsPDF('p', 'in', [w, h]),
                    text = "Promotion Distribution Report" 
                doc.addImage(dataUrl, 'JPEG', 0.13, 0.01, w, h);
                doc.save('promotion-distribution-report.pdf')
                $('#loading').hide()
            });
    }, 100)
}
		...
		setTimeout(function() {
            ...
            domtoimage.toPng(node[0])
                ...
                if(i == chartArray.length) {
                    pptx.save('promotion-distribution-report');
                    $('#loading').hide()
                }
            });
            }, 1000 * index);
...
function exportCsv(data) {
    ...
    var exportArrKeys = ['gen', 'cou', 'city', 'bl_1', 'bl_2', 'bl_3','fun', 's_fun', 's_s_fun', 'des', 'gra', 'lev', 'edu', 'cri_t', 'cri_p', 's_cat','com_j_d', 'r_cu_y', 'pr_2_y', 'urb_r_c'];
    var dataObj = {};
    var dataObjNew = [];
   ...
   $.each(data, function (key, val) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Promotion Distribution Report');
    XLSX.writeFile(wb, "promotion-distribution-report.xlsx");
}
...
var Store = function() {
    ...
    Store.prototype.buildData = function() {
        ...
        this.allRatioRange = this.getUniqueByKey(dataKeys.promotedIn2yrs);//.reverse();

        if(this.allRatioRange) {
            this.allRatioRange = this.allRatioRange.sort(this.sortNumber);
        }

        this.ratioRangeDim = this.crossEmps.dimension(function(d) { return d[dataKeys.promotedIn2yrs] })
    ...
    Store.prototype.formatTenureData = function(dim, otherDim, keys, otherKeys) {
        ...
        $.each(otherKeys, function (allRatioRangeKey, allRatioRangeVal) {
            ...
            for (var i = 0; i < dataTop.length; i++) {
                ...
                if (dataTop[i][dataKeys.promotedIn2yrs] == allRatioRangeVal) {
    ...
    Store.prototype.sortNumber = function(a, b){
        return a - b;
    }
