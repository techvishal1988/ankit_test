                ...
                if (totalUnitSalary > 0) {
                    data = _.map(data.slice(), function(d) {
                        if (obj && d.name === obj['Current_Base_salary'].name) {
                            d.data.push({
                                y: formatNumber(baseSalarySum),
                                percentage: formatNumber((baseSalarySum / totalUnitSalary) * 100)
                            })
                        }
                        if (obj && d.name === obj['Current_target_bonus'].name) {
                            d.data.push({
                                y: formatNumber(targetBonusSum),
                                percentage: formatNumber((targetBonusSum / totalUnitSalary) * 100)
                            })
                        }
                        _.map(self.allCompanyAllowance, function(all) {
                            var tSum = self.getTotalSalary(x, all.key);
                            if (d.name === all.value) {
                                d.data.push({
                                    y: formatNumber(tSum),
                                    percentage: formatNumber((tSum / totalUnitSalary) * 100)
                                })
                            }
                            return all;
                        })
...
function Provider(GRAPHAPIURL) {
    //return $.get('/src-white/data/data-paymix.json')
