...
function exportCsv(data) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Pay Range Dashboard');
    XLSX.writeFile(wb, "Payrange-dashboard.xlsx");
}
...
var dataKeys = {
    "baseSalary": "Current_Base_salary",
    "targetSalary": "Current_target_bonus",
    ...
    "country": "Country",
    "city": "City",
    "marketTargetMin": "Market_Target_Salary_Level_Min",
    "marketTargetMid": "Market_Target_Salary_Level_5",
    "marketTargetMax": "Market_Target_Salary_Level_Max",
    "marketBaseMin": "Market_Base_Salary_Level_Min",
    "marketBaseMid": "Market_Base_Salary_Level_5",
    "marketBaseMax": "Market_Base_Salary_Level_Max",
    "marketTotalMin": "Market_Total_Salary_Level_Min",
    "marketTotalMid": "Market_Total_Salary_Level_5",
    "marketTotalMax": "Market_Total_Salary_Level_Max",
}
...
var Store = function() {
    ...
    Store.prototype.buildData = function(salaryType, unitType) {
        this.unitType = unitType;
        this.salaryType = salaryType;
        if (unitType === 'internal') {
            if (salaryType === 'base_salary') this.salaryTypeInt = dataKeys.baseSalary;
            if (salaryType === 'target_salary') this.salaryTypeInt = dataKeys.targetSalary;
            if (salaryType === 'total_salary') this.salaryTypeInt = dataKeys.totalSalary;
        } else {
            if (salaryType === 'base_salary') {
                this.salaryTypeExt = 'Market_Base_Salary_Level_';
            }
            if (salaryType === 'target_salary') {
                this.salaryTypeExt = 'Market_Target_Salary_Level_';
            }
            if (salaryType === 'total_salary') {
                this.salaryTypeExt = 'Market_Total_Salary_Level_';
            }
        }
    ...
    Store.prototype.getCountryDataset = function() {
        ...
        this.countryData = this.getLineData(this.countryDim, this.countries)
    }
    ...
    Store.prototype.getCityDataset = function() {
        ...
        this.cityData = this.getLineData(this.cityDim, this.cities)
    }
    ...
    Store.prototype.getCurrentBUDataset = function() {
        ...
        this.buLevelThreeData = this.getLineData(this.buLevelThreeDim, this.buLevelThreeValues);
    }
    ...
    Store.prototype.getGradeData = function() {
        ...
        this.gradeData = this.getLineData(this.gradeDim, this.gradeKeys)
    }
    ...
    Store.prototype.getLevelData = function() {
        ...
        this.levelData = this.getLineData(this.levelDim, this.levelKeys)
    }
    ...
    Store.prototype.getFunctionData = function() {
        ...
        this.functionData = this.getLineData(this.functionDim, this.functionKeys)
    }
    ...
    Store.prototype.getPerRatingData = function() {
        ...
        this.perfRatingData = this.getLineData(this.perfRatingDim, this.prefRatingKeys)
    }
    ...
    Store.prototype.getGenderData = function() {
        ...
        this.genderData = this.getLineData(this.genderDim, this.genderKeys)
    }

    Store.prototype.getIntSalary = function(data, dataKey) {
        var sal = [];
        if (!data.length) return sal;

        for (var i = 0; i < data.length; i++) {
            sal.push(formatNumber(data[i][dataKey]))
        }

        if (sal.length > 0) {
            sal.sort(function (a, b) { return a - b });
            var min = sal[0]
            var mid = sal[Math.floor(sal.length / 2)]
            var max = sal[sal.length - 1]
            return [min, mid, max]
        }
    ...
    Store.prototype.getExtSalary = function(data, dataKey) {
        var sal = [];
        if (!data.length) return sal;
        for (var i = 0; i < data.length; i++) {
            var d = [];
            _.forIn(data[i], function(val, key) {
                if (key.indexOf(dataKey) === 0) {
                    d.push(formatNumber(val))
                }
            })
            sal = sal.concat(d);
        }
    ...
    Store.prototype.getLineData = function(dim, keys) {
        var data = []
        var labels = []
        var dataSet = {
            xAxis: {
        ...
        for (var i = 0; i < keys.length; i++) {
            labels.push(keys[i])
            dim.filter(keys[i])
            var dl = dim.top(Infinity)
            if (this.unitType === 'internal') {
                data.push({
                    name: keys[i],
                    data: this.getIntSalary(dl, this.salaryTypeInt)
                })
            } else {
                data.push({
                    name: keys[i],
                    data: this.getExtSalary(dl, this.salaryTypeExt)
                })
            }
            dim.filter(null)
        }

        dataSet.xAxis.categories = ['min', 'mid', 'max'];
        dataSet.series = data;
    ...
    Store.prototype.filterData = function(filters) {
        ...
        this.buildData(this.salaryType, this.unitType);
    }
...
function Provider(type) {
   ...
   // return $.get(type)
	$('#rpt_loading').show();
	return $.get(type).always(function(){$('#rpt_loading').hide();});
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.buLoc)
                        }
                    }
                },
                animation: {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.country)
                        }
                    }
                },
                animation: {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.city)
                        }
                    }
                },
                animation: {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.grade)
                        }
                    }
                },
                animation: {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.level)
                        }
                    }
                },
                animation: {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.function)
                        }
                    }
                },
                animation: {
...
function buildPerfRatingChart() {
    var chartOptions = _.assign({}, {
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.rating)
                        }
                    }
                },
                animation: {
        ...
        title: {
            text: 'Pay Range by Performance Rating'
        },
        ...
        plotOptions: {
            series: {
                ...
                point: {
                    events: {
                        click: function() {
                            handleChartClick(this.series.name, this.index, chartNames.gender)
                        }
                    }
                },
                animation: {
...
function handleForm(GRAPHAPIURL) {
    var salaryType = $('#salaryType'),
        unitType = $('#unitType'),
        salaryTypeValue = GRAPHAPIURL+salaryType.val(),
        unitTypeValue = unitType.val();

    salaryType.change(function() {
        salaryTypeValue = $(this).val();
        unitTypeValue = unitType.val();
        Provider(GRAPHAPIURL+salaryTypeValue).then(function(data) {
            store.employees = data.data
            store.originalEmps = data.data
            store.buildData(salaryTypeValue, unitTypeValue)
            buBarChart()
            $('.loader').hide();
            isLoading = false
        })
	...
	unitType.change(function() {
        unitTypeValue = $(this).val();
        salaryTypeValue = salaryType.val();
        Provider(GRAPHAPIURL+salaryTypeValue).then(function(data) {
            store.employees = data.data
            store.originalEmps = data.data
            store.buildData(salaryTypeValue, unitTypeValue)
    ...
    Provider(GRAPHAPIURL+salaryType.val()).then(function(data) {
        store.employees = data.data
        store.originalEmps = data.data
        store.buildData(salaryType.val(), unitTypeValue)
        buildCharts()
        $('.loader').hide();
        isLoading = false
    })
