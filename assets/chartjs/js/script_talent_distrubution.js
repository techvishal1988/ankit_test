...
function exportCsv(data) {
	...
	XLSX.writeFile(wb, "compben_dashboard_talent_rating.xlsx");
}
...
//	setValues = function (data, keys) {
//		newValues = [];
//		keys.forEach(function (key) {
//			newValues[key] = data['IT : Professional'];
//		});
//		return newValues
//	}
	...
	formDemographicData = function (data, demography) {
		...
		while (remainingData.length > 0) {
			...
			for (i = 1; i < remainingData.length; i++) {
				...
				//count = remainingData[0]['Headcount'];
...
//		},
	...
	exportToPDFBtn.click(function () {
		...
		Highcharts.exportCharts(allChartsOnThePageAsHighCharts, {
			filename : 'compben_charts_talent_rating',
...
function exportPpts() {
	...
	var chartArray = ["country-container", "city-container", "level-container", "grade-container", "function-container", "businessunit-container", "performance_rating-container", "gender-container"];
	var i = 0;
		...
		setTimeout(function() {
		  ...
		  domtoimage.toPng(node[0])
			...
			$.when( slide ).done(function ( slide ) {
			    slide.addImage({ data:dataUrl, x:2, y:0.75, w:w, h:h});
			    i++;
			});

			if(i == chartArray.length) {
			    pptx.save('compben_charts_talent_rating');
			    $('#loading').hide()
			}
