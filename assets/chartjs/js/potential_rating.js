        ...
        domtoimage.toPng(node[0])
            .then(function (dataUrl) {
                var doc = new jsPDF('p', 'in', [w, h]),
                    text = "Potential Rating Dashobard"
                doc.addImage(dataUrl, 'JPEG', 0.13, 0.01, w, h);
                doc.save('potential-rating-dashboard.pdf')
                $('#loading').hide()
            });
    }, 100)
}
		...
		setTimeout(function() {
            ...
            domtoimage.toPng(node[0])
                ...
                if(i == chartArray.length) {
                    pptx.save('potential-rating-dashboard');
                    $('#loading').hide()
                }
            });
            }, 1000 * index);
...
function exportCsv(data) {
    console.log(data);
    var exportArrKeys = ['gen', 'cou', 'city', 'bl_1', 'bl_2', 'bl_3','fun', 's_fun', 's_s_fun','des', 'gra', 'lev', 'edu', 'cri_t', 'cri_p', 's_cat','com_j_d', 'urb_r_c', 'sal_lv'];
    var dataObj = {};
    var dataObjNew = [];
   ...
   $.each(data, function (key, val) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Potential Rating Dashobard');
    XLSX.writeFile(wb, "potential-rating-dashboard.xlsx");
}
...
var Store = function() {
    ...
    Store.prototype.buildData = function() {
        ...
        this.allRatioRange = this.getUniqueByKey(dataKeys.perfRating);
        this.ratioRangeDim = this.crossEmps.dimension(function(d) { return d[dataKeys.perfRating] })
    ...
    Store.prototype.formatTenureData = function(dim, otherDim, keys, otherKeys) {
        ...
        $.each(otherKeys, function (allRatioRangeKey, allRatioRangeVal) {
            ...
            for (var i = 0; i < dataTop.length; i++) {
                ...
                if (dataTop[i][dataKeys.perfRating] == allRatioRangeVal) {
