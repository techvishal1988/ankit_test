...
var chartIndexes = ['#cityChart', '#businessUnit1', '#hcByGrade', '#hcByLevel', '#hcByFunction', '#perfRating', '#genderChart']

function createPdfPages(pdf) {
    if (chartIndexes.length) {
        var chart = $(chartIndexes.shift()).closest('.col-md-6'),
...
function exportCharts(charts) {
    ...
    window.setTimeout(function() {
        ...
        //         doc.save('salary-analysis-dashboard.pdf')
        //         $('.loader').hide()
        //     });
    }, 100)
}
    ...
    Store.prototype.getBarPercentage = function(dim, keys, otherKeys) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (dl > 0) {
                ...
                var min = _.minBy(fValues, function(m) {
                    return parseInt(m[dataKeys.salary_increase], 10)
                })
                var max = _.maxBy(fValues, function(m) {
                    return parseInt(m[dataKeys.salary_increase], 10)
                })
                var avg = formatNumber(_.sumBy(fValues, function(m) {
                    return parseInt(m[dataKeys.salary_increase], 10)
                }) / dl)
                if (min) {
                    totalUnitSalary += formatNumber(min[dataKeys.salary_increase])
                }
                if (max) {
                    totalUnitSalary += formatNumber(max[dataKeys.salary_increase])
                }
                totalUnitSalary += avg;
                data = _.map(data.slice(0), function(item) {
                    if (item.name === 'min' && min) {
                        var p = (parseFloat(min[dataKeys.salary_increase], 10) / totalUnitSalary) * 100
                        item.data.push(formatNumber(p))
                    }
                    if (item.name === 'avg' && avg) {
                        var p = (parseFloat(avg, 10) / totalUnitSalary) * 100
                        item.data.push(formatNumber(p))
                    }
                    if (item.name === 'max' && max) {
                        var p = (parseFloat(max[dataKeys.salary_increase], 10) / totalUnitSalary) * 100
                        item.data.push(formatNumber(p))
                    }
                    return item
                })
...
function buildCountryChart() {
    var chartOptions = _.assign({}, {
        ...
        title: {
            text: 'Salary Increase by Country'
        },
    }, store.countryData)
...
function buildFunctionChart() {
    var chartOptions = _.assign({}, {
        ...
        title: {
            text: 'Salary Increase by Function'
        },
    }, store.functionData)
...
function buildPerfRatingChart() {
    var chartOptions = _.assign({}, {
        ...
        title: {
            text: 'Salary Increase by Performance Rating'
        },
...
function buildGenderChart() {
    var chartOptions = _.assign({}, {
        ...
        title: {
            text: 'Salary Increase by Gender'
        },
    }, store.genderData)
