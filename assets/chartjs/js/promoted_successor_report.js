        ...
        domtoimage.toPng(node[0])
            .then(function (dataUrl) {
                var doc = new jsPDF('p', 'in', [w, h]),
                    text = "Successor's who got promoted"
                doc.addImage(dataUrl, 'JPEG', 0.13, 0.01, w, h);
                doc.save('successors-who-got-promoted-dashboard.pdf')
                $('#loading').hide()
            });
    }, 100)
}
		...
		setTimeout(function() {
            ...
            domtoimage.toPng(node[0])
                ...
                if(i == chartArray.length) {
                    pptx.save('successors-who-got-promoted-dashboard');
                    $('#loading').hide()
                }
            });
            }, 1000 * index);
...
function exportCsv(data) {
    ...
    'company_joining_date', 'rating_for_current_year', 'promoted_in_2_yrs', 'successor_identified' ];
    var dataObj = {};
    var dataObjNew = [];
   ...
   $.each(data, function (key, val) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Successor's who got promoted");
    XLSX.writeFile(wb, "successors-who-got-promoted-dashboard.xlsx");
}
