        ...
        domtoimage.toPng(node[0])
            .then(function (dataUrl) {
                var doc = new jsPDF('p', 'in', [w, h]),
                    text = "Identified Successor's Dashobard"
                doc.addImage(dataUrl, 'JPEG', 0.13, 0.01, w, h);
                doc.save('identified-successors-dashboard.pdf')
                $('#loading').hide()
            });
    }, 100)
}
		...
		setTimeout(function() {
            ...
            domtoimage.toPng(node[0])
                ...
                if(i == chartArray.length) {
                    pptx.save('identified-successors-dashboard');
                    $('#loading').hide()
                }
            });
            }, 1000 * index);
...
function exportCsv(data) {
    ...
    'company_joining_date', 'rating_for_current_year', 'successor_identified'];
    var dataObj = {};
    var dataObjNew = [];
   ...
   $.each(data, function (key, val) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Identified Successors Dashobard');
    XLSX.writeFile(wb, "identified-successors-dashboard.xlsx");
}
...
var Store = function() {
    ...
    Store.prototype.buildData = function() {
        ...
        this.allRatioRange = this.getUniqueByKey(dataKeys.successorIdentified).reverse();
        this.ratioRangeDim = this.crossEmps.dimension(function(d) { return d[dataKeys.successorIdentified] })
    ...
    Store.prototype.formatTenureData = function(dim, otherDim, keys, otherKeys) {
        ...
        $.each(otherKeys, function (allRatioRangeKey, allRatioRangeVal) {
            ...
            for (var i = 0; i < dataTop.length; i++) {
                ...
                if (dataTop[i][dataKeys.successorIdentified] == allRatioRangeVal) {
