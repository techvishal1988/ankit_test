<style type="text/css">
/*.mailbox-content.manager_landing table thead tr th.width-100{width:200px !important;}	*/
</style>
    	...
    	<div>
			...
			if($staff_list)
			{
				...
				<div class="table-scroll">
					<table id="table_<?=$newId?>" class="table border">
						<thead class="emp_list_head">
							<tr>
								<?php ##################### Start :: Dynamic Header Goes Here ###################							
								foreach ($table_atrributes as $row)
								{							
									if($row->attribute_name=='manager_discretions')
									{							
										echo '<th style="width:6.9%;" class="showfilter">'.$row->display_name.'<a href="JavaScript:Void(0);" data-placement="bottom" data-toggle=tooltip data-original-title="The salary recommendation reflected here is based on the rules setup for this compensation review plan by HR. However, you may have rights and scope to review the budget of your team members within certain limits. Please check the “Available budget” on the top right hand side of this screen and click the edit button if you like to modify any salary recommendations"><i class="fa fa-info-circle themeclr"></i></a> <br><span class="original_recomd">'.'Original Recommendation</span></th>'; 
									} 
									else if($row->attribute_name == "salary_increment_amount")
									{
										echo '<th>'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "merit_increase_amount")
									{
										echo '<th class="grey_bg" >'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "desig")
									{
										echo '<th style="min-width: 200px !important;" class="showfilter width-100">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "function")
									{
										echo '<th style="min-width: 160px !important;" class=" showfilter width-100">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "subfunction")
									{
										echo '<th style="min-width: 160px !important;" class="showfilter width-100">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "sub_sub_function")
									{
										echo '<th style="min-width: 160px !important;" class="showfilter width-100">'.$row->display_name.'</th>';
									}
									
									else if($row->attribute_name == "merit_increase_percentage")
									{
										echo '<th class="grey_bg">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "market_correction_amount")
									{
										echo '<th class="blue_bg">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name == "market_correction_percentage")
									{
										echo '<th  class="blue_bg">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name=='promotion_recommandetion') 
									{
										echo '<th class="redish_bg">Promotion<br>Recommendation</th>';
									}
									else if($row->attribute_name=='sp_manager_discretions') 
									{
										echo '<th class="redish_bg">'.$row->display_name.'</th>';
									}		
									else if($row->attribute_name=='promotion_comment') 
									{
										echo '<th class="redish_bg">'.$row->display_name.'</th>';
									}
									else if($row->attribute_name=='position_pay_range_after_promotion') 
									{
										echo '<th class="redish_bg">'.$row->display_name.'</th>';
									}														
									else if($row->attribute_name=='sp_increased_salary')
									{							
										echo '<th class="redish_bg">Promotion Increase amount<a href="JavaScript:Void(0);" data-placement="bottom" data-toggle="tooltip" data-original-title="" title="All promotion recommendations are subject to approval of the management. Please provide a valid reason for recommending promotion in your team to support your case."><i class="fa fa-info-circle themeclr"></i></a></th>';
									}
									else if($row->attribute_name == "new_".$promotion_col_name)
									{
										echo '<th class="redish_bg">'.$row->display_name.'</th>';
									}
									else if(in_array($row->attribute_name,$other_then_promotion_col_arr))
									{
										echo '<th class="redish_bg">'.$row->display_name.'</th>';							
									}
									else if($row->attribute_name == "other_data_9" or $row->attribute_name == "other_data_10" or $row->attribute_name == "other_data_11")
									{
										echo '<th class="'.$rsu_color_cls.'">'.$row->display_name.'</th>';
									}
									...
									/*
									else if(in_array($row->attribute_name,[
										'name','grade','level','employee_type','country','city','business_level_1','business_level_2','business_level_3',
									'critical_talent','special_category','successor_identified','readyness_level',
									'performance_rating','rating_last_year','urban_rural_classification']))
									{
										echo '<th class="showfilter">'.$row->display_name.'</th>';
									}*/
									else
									{
										if($row->attribute_name!='new_designation' && $row->attribute_name!='new_grade' && $row->attribute_name!='new_level')
										{							
											echo '<th class="showfilter1 '.$row->attribute_name.'">'.$row->display_name.'</th>';
										}							
									}
								} ?>
								...
								{
									echo '<th class="'.$rsu_color_cls.'">Target RSU Amount</th>';
								} ?>
								<!-- Note :: End: RSU Section For Fresh Work Only -->
								
								<?php if($rule_dtls['esop_title']!="" && !empty($checkApproverEsopRights)) {?>
									<th class='<?php echo $rsu_color_cls; ?>'><?php echo $rule_dtls['esop_title']; ?></th>
								<?php } ?>
								
								<!-- Note :: Start :: RSU Section For Fresh Work Only -->
								<?php if(in_array($this->session->userdata('domainname_ses'), $rsu_enabled_for_domains_arr))
								{
									echo '<th class="'.$rsu_color_cls.'">Final Amount of RSU</th>';
									echo '<th class="'.$rsu_color_cls.'">Final RSU</th>';
								} ?>
								<!-- Note :: End: RSU Section For Fresh Work Only -->
								...
								<?php } ##################### End :: Dynamic Header End Here ################### ?>
						...
						<tbody id="tbl_body">                   
							...
							foreach($staff_list as $row)
							{
								...
								echo '<tr id="tbl_tr_'.$row['tbl_pk_id'].'">';
								foreach($table_atrributes as $key)
								{
									if($key->attribute_name=='name')
									{
										...
										echo "<td class='tdgrey'><a target='_blank' href=".site_url("view-employee/".$row["id"]).">".$row["name"]."</a><a class='pull-left view_dash' data-placement='right' data-toggle='tooltip' data-original-title='See Analytics' href=".site_url($view_emp_increments_dtls_path."/".$rule_id."/".$row["id"])."><i class='fa fa-qrcode themeclr' aria-hidden='true'></i></a>";
									...
									{
										echo "<td class='text-center tdgrey'>".$row[$key->attribute_name]."</td>";
									}
									...
									else if($key->attribute_name=='position_pay_range_after_promotion')
									{
										...
										{
											echo "<td class='txtalignright redish_bg'><b>".$row["crr_after_promotion"]."%</b></td>";
										}										
									}
									...
									{	
										...
										{
											echo "<td class='txtalignright grey_bg'><span class='font_sz14'><b>".HLP_get_formated_amount_common($merit_increase_amount)."</b></span></td>";									
										}
									}
									...
									{										
										...
										{
											echo "<td class='txtalignright blue_bg'><span class='font_sz14'><b>".HLP_get_formated_amount_common($market_correction_amount)."</b></span></td>";									
										}
									}
									elseif($key->attribute_name=='market_correction_percentage')
									{
										...
										if($is_user_can_edit_salary_hikes==1 and in_array(CV_SALARY_MARKET_HIKE, $type_of_hike_can_edit_arr))
										{
											...
											<input class='form-control' onKeyUp='validate_percentage_onkeyup_common(this,3);' type='text' value=".HLP_get_formated_percentage_common($row["final_market_hike"])." id='salary_per_market".$row['tbl_pk_id']."' style='display:none;' onblur='getamt(\"market\",this.value,".$row['tbl_pk_id'].",".($row["increment_applied_on_salary"]).")' />
											<input type='hidden' value=".$row["final_market_hike"]." id='salary_per_hidden_market".$row['tbl_pk_id']."' />
											<span class='fa fa-edit salary_edit_market".$row['tbl_pk_id']."' onclick='salary_edit(\"market\",".$row['tbl_pk_id'].",".($row["increment_applied_on_salary"]).");'>
											</span>
											<span class='fa fa-close salary_close_market".$row['tbl_pk_id']."' onclick='salary_close(\"market\",".$row['tbl_pk_id'].",".($row["increment_applied_on_salary"]).");' style='display:none;'></span></div></td>";
										}
										else
										{							
									...
									else if($key->attribute_name=='salary_increment_amount')
									{
										...
										{
											echo "<td class='txtalignright'><span class='font_sz14'><b>".HLP_get_formated_amount_common($salary_increment_amount)."</b></span></td>";									
										}
									}
											...
											<div class="beg_color">
												<label style="color:#000 !important; margin-bottom: 0px !important;">
													...
													if($is_chk_check == "checked"){$is_chk_editable = 'disabled';}
													 ?>
													<?php /*?><input type="checkbox" id="chk_dfault_promotion<?php echo $row['tbl_pk_id']; ?>" style="opacity:1; margin-top:1px;" <?php echo $is_chk_check; ?> <?php echo $is_chk_editable; ?> onclick="set_default_promotion_to_emp('<?php echo $row['tbl_pk_id']; ?>', '<?php echo $row["standard_promotion_increase"]; ?>', '<?php echo $rule_dtls['manager_can_exceed_budget']; ?>');"><?php */?>
									...
									{ ?>								
										<td class="txtalignright redish_bg">
											<div class="edit_recommd">
												...
												<input class="form-control" onKeyUp="validate_onkeyup_num(this);" id="promotion_increment<?php echo $row['tbl_pk_id']; ?>" maxlength="10" type="text" value="<?php echo round($row["sp_increased_salary"]); ?>" style="display:none;" onblur="getperr(this.value,'<?php echo $row['tbl_pk_id']; ?>','<?php echo ($row["increment_applied_on_salary"]); ?>')" <?=$readOnly?>>
												
												<?php /*?><?php if($is_user_can_edit_salary_hikes){ ?>
													<span class='fa fa-edit promotion_edit<?php echo $row['tbl_pk_id']; ?>' onclick='promotion_edit("<?php echo $row['tbl_pk_id']; ?>","<?php echo $row["standard_promotion_increase"]; ?>","<?php echo $rule_dtls['manager_can_exceed_budget']; ?>");'>
													</span>
													<?php if($sp_per>0){ ?>
														<span class='fa fa-minus promotio_del<?php echo $row['tbl_pk_id']; ?>' onclick='deletepromotion("<?php echo $row['tbl_pk_id']; ?>");' style='display:none;'></span>
													<?php } ?>
													<span class='fa fa-close promotion_close<?php echo $row['tbl_pk_id'] ?>' onclick='promotion_close("<?php echo $row['tbl_pk_id']; ?>","<?php echo $row["standard_promotion_increase"]; ?>" ,"<?php echo $rule_dtls['manager_can_exceed_budget']; ?>");' style='display:none;'></span>
											<?php } ?><?php */?>
											</div>
										</td>								
									<?php 
									...
									{  ?>
										<td class="txtalignright redish_bg">
											<div class="edit_recommd">
												...
												<input class="form-control" onKeyUp="validate_percentage_onkeyup_common(this,3);" id="promotion_per<?php echo $row['tbl_pk_id']; ?>" type="text" value="<?php if($row['emp_new_'.$promotion_col_name]>0){ echo HLP_get_formated_percentage_common($sp_per);}else{ echo HLP_get_formated_percentage_common($row['standard_promotion_increase']); } ?>" style="display: none;" onblur="getamtt(this.value,'<?php echo $row['tbl_pk_id']; ?>','<?php echo ($row["increment_applied_on_salary"]); ?>')" <?=$readOnly?>>
												<input id="promotion_per_hidden<?php echo $row['tbl_pk_id']; ?>" type="hidden" value="<?php if($row['emp_new_'.$promotion_col_name]>0){ echo HLP_get_formated_percentage_common($sp_per);} else { echo HLP_get_formated_percentage_common($row['standard_promotion_increase']); }  ?>" style="display: none;" onblur="getamtt(this.value,'<?php echo $row['tbl_pk_id']; ?>','<?php echo ($row["increment_applied_on_salary"]); ?>')">
												<?php /*?><?php if($is_user_can_edit_salary_hikes){ ?>
													<span class='fa fa-edit promotion_edit<?php echo $row['tbl_pk_id']; ?>' onclick='promotion_edit("<?php echo $row['tbl_pk_id']; ?>","<?php echo $row["standard_promotion_increase"]; ?>","<?php echo $rule_dtls['manager_can_exceed_budget']; ?>");'></span>
													<?php if($sp_per>0){ ?>
														<span class='fa fa-minus promotio_del<?php echo $row['tbl_pk_id']; ?>' onclick='deletepromotion("<?php echo $row['tbl_pk_id']; ?>");' style='display:none;'></span>
													<?php } ?>
													<span class="fa fa-close promotion_close<?php echo $row['tbl_pk_id']; ?>" onclick="promotion_close("<?php echo $row['tbl_pk_id']; ?>","<?php echo $row["standard_promotion_increase"]; ?>" ,"<?php echo $rule_dtls['manager_can_exceed_budget']; ?>");" style="display:none;"></span>
