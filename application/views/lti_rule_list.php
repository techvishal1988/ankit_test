    ...
    <div class="row">
      <div class="col-sm-8">
       <ol class="breadcrumb wn_btn" style="padding-top: 6px !important;">
        ...
        <li class="active">LTI Rules</li>
      </div>
      <div class="col-sm-4">
        <?php if(helper_have_rights(CV_COMP_LTI, CV_INSERT_RIGHT_NAME)) { ?>
        <div class="pull-right">
            <a href="<?php echo site_url("lti-rule-filters/".$performance_cycle_id); ?>"><button class="btn btn-success create_new_rule_btn" type="button">Create New LTI Rules</button></a>
        </div>
         <?php } ?>
       </div>
       </div>
...
<?php echo $this->session->flashdata('message');
	  ...
	  $is_show_null_record_msg = 1; ?>
   	<?php if($lti_rule_list){
		 $is_show_null_record_msg = 0; ?>
        <!--<p>LTI Rule List Created By Self</p>-->
         ...
         <?php $i=0;
             ...
             foreach($lti_rule_list as $row)
             {
				...
				if($row["createdby"] == $this->session->userdata("userid_ses"))
				{
					...
					{
						echo "<a data-toggle='tooltip' data-original-title='Edit Rule' data-placement='bottom' href='".site_url("lti-rule-filters/$performance_cycle_id/$id")."'alt='Edit Rule' title='Edit Rule'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a>";
						
						 /*| <a data-toggle='tooltip' data-original-title='Delete' data-placement='bottom' onclick="return confirm('Are you sure, You want to delete rule?')" href="<?php echo site_url("delete-lti-rule/$id"); ?>" alt="Delete" title="Delete"><i class='fa fa-trash' aria-hidden='true'></i></a>*/
											
					}
					elseif($row["status"] == 3)
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-lti-rule-details/$id")."' alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='Edit Rule' data-placement='bottom' href='".site_url("lti-rule-filters/$performance_cycle_id/$id")."'alt='Edit Rule' title='Edit Rule'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='Send for Approval' data-placement='bottom' href='".site_url("view-lti-rule-details/$id")."'alt='Send for Approval' title='Send for Approval''><i class='fa fa-thumbs-o-up' aria-hidden='true'></i></a>";
					...
					elseif($row["status"] == 4)
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-lti-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='View Incentive List' data-placement='bottom' href='".site_url("view-lti-incentive-list/$id")."'alt='View Incentive List' title='View Incentive List'><i class='fa fa-list' aria-hidden='true'></i></a>";
					...
					{						
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-lti-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='View Incentive List' data-placement='bottom' href='".site_url("view-lti-incentive-list/$id")."'alt='View Incentive List' title='View Incentive List'><i class='fa fa-list' aria-hidden='true'></i></a> | ";
						echo "<a data-toggle='tooltip' data-original-title='Edit LTI Stock Values Of Vestings' data-placement='bottom' href='".site_url("edit-lti-stock-values/$id")."'alt='Edit LTI Stock Values Of Vestings' title='Edit LTI Stock Values Of Vestings'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a>";
						
					}
					else
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-lti-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a>";
					} 
					
					if($row["status"] < 9)
					{?>
						 | <a class="anchor_cstm_popup_cls_lti_delete<?php echo $id; ?>" data-toggle='tooltip' data-original-title='Delete' data-placement='bottom' onclick="return  request_custom_anchor_confirm('anchor_cstm_popup_cls_lti_delete<?php echo $id; ?>','<?php echo $this->lang->line('msg_critical_delete_record'); ?>')" href="<?php echo site_url("delete-lti-rule/$id"); ?>" alt="Delete" title="Delete"><i class='fa fa-trash' aria-hidden='true'></i></a>
				<?php }
				...
				else
				{
					...
					if($this->session->userdata('role_ses') <= 2 and $row["status"] < 9)
					{?>
						<a class="anchor_cstm_popup_cls_lti_delete<?php echo $id; ?>" data-toggle='tooltip' data-original-title='Delete' data-placement='bottom' onclick="return  request_custom_anchor_confirm('anchor_cstm_popup_cls_lti_delete<?php echo $id; ?>','<?php echo $this->lang->line('msg_critical_delete_record'); ?>')" href="<?php echo site_url("delete-lti-rule/$id"); ?>" alt="Delete" title="Delete"><i class='fa fa-trash' aria-hidden='true'></i></a>
				<?php }
				}
				...
				if($row["status"] >= 3)
				{
					echo ' | <a target="_blank" data-toggle="tooltip" data-original-title="Print rule" data-placement="bottom" href="'.base_url("print-preview-lti/".$id).'"><i class="fa fa-print" aria-hidden="true"></i></a>';
				}
    ...
    <?php } ?> 
   	...
   	<?php if($lti_approvel_request_list){
		 $is_show_null_record_msg = 0; ?>
       
        <p>Pending Approval List of LTI Rules</p>
        <div class="table-responsive">
         <table id="example" class="table border" style="width: 100%; cellspacing: 0;">
            ...
            <tbody id="tbl_body">                   
             <?php $i=0;
             foreach($lti_approvel_request_list as $row)
             {
                ...
                if($row["type"]==4)
                {
                    echo "<a href='".site_url("view-lti-rule-details/$rule_id")."'>Approve LTI Rule</a>";
                }                
                echo "</td>";
                echo "</tr>";
                $i++;
    ...
    <?php $is_show_release_list = 0; 
        ...
        <p>Pending Rule Release List of LTI Rules</p>
            ...
            <tbody id="tbl_body">                   
             ...
             foreach($rules_release_approvel_list as $row)
             {
				if($row["total_emp_in_rule"]==$row["total_appoved_emp_in_rule"])
                {
					...
					<form onsubmit="return confirm('Are you sure, You want to release the rule ?')" method="post" action="">
                    	<?php echo HLP_get_crsf_field();?>
...
function openEmailTemplatePopup(ruleId) {
 ...
 var csrf_val = $('input[name=csrf_test_name]');
        $.ajax({
            url: '<?Php echo site_url('performance_cycle/getEmailLtiRuleTemplate'); ?>',
            type: 'get',
            ...
            success: function (res) {
              ...
              {
                  ancoreTag='<a class="btn btn-primary" onclick="loaderActivate()" href="<?=site_url("sentEmailLtiToApprove/")?>'+ruleId+'">Send Mail</a>';
              ...
              {
                  ancoreTag='<a class="btn btn-primary" onclick="loaderActivate()" href="<?=site_url("reminderEmailLtiToApprovers/")?>'+ruleId+'">Send Reminder Mail</a>';
