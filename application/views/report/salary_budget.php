  ...
  <div class="row">
   <div class="col-sm-8">
    <ol class="breadcrumb">
        ...
        <li class="active"><span>Salary Increase Budget Allocation vs Utilisation Report</span></li>
    </ol>
   </div> 
...
<div class="container-fluid compp_fluid">
    <div class="text-center bgclsgraph" style="padding-bottom: 0px; margin-bottom: 0px;">
        ...
        $tooltip=getToolTip('comppositining-report');
              ...
              ?>
        <h3 style="padding: 15px" class="h3grapg text20 textbold">Salary Increase Budget Allocation vs Utilisation in <strong class="currency-format"> </strong> <strong class="total-emp"></strong>
...
?>
<script src="<?php echo base_url('assets/chartjs') ?>/js/script-salary-budget.min.js"></script>
...
function GetData(val)
{
    ...
    if(val==='')
    {
        ...
        $('.datacontainer').show();
        var GRAPHAPIURL="<?php echo base_url('api/graph/compposition/') ?>"+val + '/0/budget';
        App(GRAPHAPIURL);
    }
}
    $(document).ready(function() {
		...
		<?php if($this->uri->segment(3)>0 and $this->uri->segment(4)>0){?>
				...
				$('.datacontainer').show();
				App("<?php echo base_url('api/graph/compposition/'.$this->uri->segment(3).'/'.$this->uri->segment(4)).'/budget' ?>");
		<?php }else{ ?>
