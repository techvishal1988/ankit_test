...
<div class="container-fluid compp_fluid">
    <div class="text-center bgclsgraph" style="padding-bottom: 0px; margin-bottom: 0px;">
        ...
        $tooltip=getToolTip('comppositining-report');
              ...
              ?>
        <h3 style="padding: 15px" class="h3grapg text20 textbold">Population Distribution in Different Comp Ranges After Increase for <strong class="total-emp"></strong><!-- <span style=" float: none;" type="button" class="tooltipcls" data-toggle="tooltip control-label" data-placement="bottom" title="" data-original-title="<?php //echo $val[0] ?>"><i class="fa fa-info-circle themeclr" aria-hidden="true"></i></span> -->
        <?php //echo '<pre >'; print_r($salary_cycles_list) ?>
...
<script>
var dataKeys = {
    "ratio_range"   : "pre_crr_range",
    "pre_ratio_range": "comparative_ratio_range",
...
?>
<script src="<?php echo base_url('assets/chartjs') ?>/js/script-composition.js"></script>
...
function GetData(val)
{
    ...
    if(val==='')
    {
        ...
        $('.datacontainer').show();
        var GRAPHAPIURL="<?php echo base_url('api/graph/compposition/') ?>"+val;
        App(GRAPHAPIURL);
    }
}
    $(document).ready(function() {
		...
		<?php if($this->uri->segment(3)>0 and $this->uri->segment(4)>0){?>
				...
				$('.datacontainer').show();
				App("<?php echo base_url('api/graph/compposition/'.$this->uri->segment(3).'/'.$this->uri->segment(4)) ?>");
		<?php }else{ ?>
