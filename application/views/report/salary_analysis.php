  ...
  <div class="row">
   <div class="col-sm-8">  
    <ol class="breadcrumb">
        ...
        <li class="active">Salary Increase Range report</li>
    </ol>
   </div>
...
<div class="container-fluid compp_fluid">
    <div class="text-center bgclsgraph" style="padding-bottom: 0px; margin-bottom: 0px;">
        <?php 
        $tooltip=getToolTip('salary-analysis-report');
              $val=json_decode($tooltip[0]->step);
              ?>
        <h3 style="padding: 15px" class="h3grapg text20 textbold">Salary Increase Range Report for<strong class="total-emp"></strong><!-- <span style=" float: none;" type="button" data-toggle="tooltip control-label" data-placement="bottom" title="" data-original-title="<?php //echo $val[0] ?>"><i style="font-size: 20px;" class="fa fa-info-circle themeclr" aria-hidden="true"></i></span> -->
         
         <select name="rules" id="rules" class="form-control mt-3 report-selected-text" style="width:auto; display: inline-block;" onchange="GetData(this.value)">
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-3 text-right">
                <button id="exportPpts" class="btn btn-default">
                    <img src="<?php echo base_url('assets/reports') ?>/icons/ppt-icon.png" alt="download pdf">
                </button>
...
?>
<script src="<?php echo base_url('assets/chartjs') ?>/js/script-salaryanalysis.js"></script>
    ...
    function GetData(val)
{
    ...
    if(val==='')
    {
        ...
        $('.datacontainer').show();
        var GRAPHAPIURL="<?php echo base_url('api/graph/compposition/') ?>"+ val + '/0/salary_analysis';
         App(GRAPHAPIURL);
    }
}
...
//        App();
        <?php if($this->uri->segment(3)>0 and $this->uri->segment(4)>0){?>
				...
				$('.datacontainer').show();
				App("<?php echo base_url('api/graph/compposition/'.$this->uri->segment(3).'/'.$this->uri->segment(4)).'/salary_analysis' ?>");
		<?php }else{ ?>
