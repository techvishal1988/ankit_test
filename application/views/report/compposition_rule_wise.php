  ...
  <div class="row">
   <div class="col-sm-8">
    <ol class="breadcrumb">
        ...
        <li class="active"><span>Comp Positioning report</span></li>
    </ol>
   </div> 
...
<div class="container-fluid compp_fluid">
    <div class="text-center bgclsgraph" style="padding-bottom: 0px; margin-bottom: 0px;">
         <?php 
              ...
              ?>
        <h3 style="padding: 15px" class="h3grapg text20 textbold">Population Distribution in Different Comp Ranges After Increase for <strong class="total-emp"></strong>
        <select name="rules" id="rules" class="form-control report-selected-text" style="width:auto; display: inline-block;" onchange="getData(this.value)">
            <?php foreach($salary_rule_list as $rule_det) { ?>
            <option value="<?php echo $rule_det['plan_rule_id'] ?>"><?php echo $rule_det['salary_rule_name'] ?></option>
            <?php } ?>
        </select>
        </h3>
    </div>
...
var dataKeys = {
    "post_ratio_range"   : "post_c_rng",
    "pre_ratio_range": "pre_c_rng",
    "businessUnit1" : "bl_1",
    "businessUnit2" : "bl_2",
    "businessUnit3" : "bl_3",
    "education"     : "edu",
...
?>
<script src="<?php echo base_url('assets/chartjs') ?>/js/script_composition_rule_wise.js"></script>
<script>
function getData(val)
{
    var result  =   val.split('<?php echo $implode_variable; ?>');
    var ruleID = result['1'];
    var cycleId = result['0'];
    $('.datacontainer').hide();
    if(val==='')
    {
        custom_alert_popup('Please select rule.');
    }
    else {
        $('.datacontainer').show();
        var GRAPHAPIURL="<?php echo base_url('api/graph/compposition_rule_wise/') ?>"+cycleId+'/'+ruleID;
        App(GRAPHAPIURL);
    }
}
$(document).ready(function() {
    <?php if($this->uri->segment(3)>0 and $this->uri->segment(4)>0){?>
            $("#rules").val("<?php echo $this->uri->segment(3).'&&'.$this->uri->segment(4); ?>");
            $('.datacontainer').show();
            App("<?php echo base_url('api/graph/compposition_rule_wise/'.$this->uri->segment(3).'/'.$this->uri->segment(4)) ?>");
    <?php }else{ ?>
