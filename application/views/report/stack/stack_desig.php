...
<?php //echo '<pre />'; print_r($res['DesignationTitle']) ?>
</div>
<?php echo @$msg ?>
<div id="graph"></div>
<style>
...
<script>
 function wrap(text, width) {
    ...
    dataset = [
        <?php foreach($res['DesignationTitle'] as $k) { ?>
        {label:"<?php echo $k['DesignationTitle'] ?>", "Minimum":<?php echo $k['Minimum'] ?>, "Average":<?php echo $k['avg'] ?>, "Maximum": <?php echo $k['Maximum'] ?>},
        <?php } ?>
...
function getType(val)
 {
	...
	if(val==1)
	{
		urll='<?php echo base_url() ?>stack/grade/'+val;
	}
	if(val==2)
	{
		urll='<?php echo base_url() ?>stack/level/'+val;
	}
	if(val==3)
	{
		urll='<?php echo base_url() ?>stack/designation/'+val;
	}
