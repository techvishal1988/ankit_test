 ...
 function wrap(text, width) {
    ...
    dataset = [
        <?php foreach($res as $k) {
            ...
            if($k['name']=='' ) {
                $name=$k['performance_rating'];
            }
            else {
                $name=$k['name'];
            }
            ?>
        {label:"<?php echo $name ?>", "Minimum":<?php echo $k['Minimum_hike'] ?>, "Average":<?php echo $k['Average_hike'] ?>, "Maximum": <?php echo $k['Maximum_hike'] ?>},
        <?php } ?>
...
function getType(val)
 {
	...
	if(val==1)
	{
		urll='<?php echo base_url() ?>stack/salary_increase_both/'+val;
	}
	if(val==2)
	{
		urll='<?php echo base_url() ?>stack/get_salary_increase_rpt_data_function/'+val;
	}
	if(val==3)
	{
		urll='<?php echo base_url() ?>stack/get_salary_increase_rpt_data_rating/'+val;
	}
