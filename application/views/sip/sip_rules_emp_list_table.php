  ...
  <div class="form-horizontal">
    <div id="dv_partial_page_data" style="overflow-x: auto;">
      ...
      <table id="example" class="table border  ">
        <thead>
                ...
                <b>
                    ...
                    foreach($managers_arr as $key => $value)
                    {
                      if(strtolower($staff_list[0]["last_action_by"]) == strtolower($value[0]))
                      {
                        ...
                        $used_budget = HLP_get_managers_emps_total_budget($rule_dtls['id'], strtolower($value[0]));
                        //echo  " , Available Budget : ".HLP_get_formated_amount_common($manager_tot_bdgt - $used_budget). ")";
						echo  " , Available Budget : 0)";
                        break;
                      }
                    } ?>
                </b>
           ...
           <!-- <th>Plan Name</th>
            ...
            <th><?php echo CV_BONUS_SIP_LABEL_NAME; ?> %age</th>
            <th><?php echo CV_BONUS_SIP_LABEL_NAME; ?> Amount</th>
            <th><?php echo $business_attributes[17]["display_name"]; ?> for <br /> <?php echo CV_BONUS_SIP_LABEL_NAME; ?> pro-rated calculation</th>
            <?php /*?><th><?php echo $business_attributes[133]["display_name"]; ?></th><?php */?>
            <!--<th> Manager Name </th>
            <th> Manager Email </th>-->
            <th> Action </th>
          </tr>
        ...
        <tbody id="tbl_body">
          <?php $i=0; $is_enable_approve_btn=1;
                        ...
                        if($prev_record_manager_email != "" and strtolower($row["last_action_by"]) != $prev_record_manager_email)
                        {
                            ...
                            {?>
                              <tr>
                                ...
                                <form id="<?php echo $i; ?>" action="<?php echo site_url("manager/dashboard/reject_emp_sip_increment"); ?>" method="post" class="form-inline">
                                    ...
                                    <div class="row" id="dv_remark_<?php echo $i; ?>" style="display: none;">
                                      <div class="col-sm-12">
                                        <div class="form-input">
                                          <input type="hidden" name="emp_sip_dtl_tbl_pk_ids" value="<?php echo implode(",",$pk_ids); ?>" />
                        ...
                        <table class='table border'>
                          <thead>
                                ...
                                <b>
                                    ...
                                    foreach($managers_arr as $key => $value)
                                    {
                                      if(strtolower($row["last_action_by"]) == strtolower($value[0]))
                                      {
                                        $manager_tot_bdgt = $value[1] + (($value[1]*$value[2])/100);
                                        echo "</b><b style='float:right;'> (Total Budget :: " . HLP_get_formated_amount_common($manager_tot_bdgt);
                                        $used_budget = HLP_get_managers_emps_total_budget($row['rule_id'], strtolower($value[0]));
                                        //echo  " , Available Budget :: ".HLP_get_formated_amount_common($manager_tot_bdgt - $used_budget). ")";
										echo  " , Available Budget :: 0)";
                                        break;
                                      }
                            ...
                            <tr>
							  ...
							  <?php if($performance_type==2){echo $business_attributes[133]["display_name"];}else{echo $business_attributes[23]["display_name"];} ?></th><?php */?>
                              <!--<th> Manager Name </th>
                              <th> Manager Email </th>-->
                              <th> Action </th>
                            </tr>
                  ...
                  <?php }
                        ...
                        if(strtolower($row["manager_emailid"]) == $manager_emailid and $row["status"] > 1 and $row["status"] < 5 and $this->session->userdata('is_manager_ses') == 1 and isset($is_open_frm_manager_side))
                        {
                            $pk_ids[] = $row["id"];
                        }
                        ...
                        echo "<td><a target='_blank' href=".site_url("view-employee/".$row["user_id"]).">".$row["emp_name"]."</a></td>";
                        echo "<td>".$row["business_level_3"]."</td>";
                        echo "<td>".$row["designation"]."</td>";
                        echo "<td>".$row["grade"]."</td>";
                        echo "<td>".$row["level"]."</td>";
                        echo "<td class='txtalignright'>".HLP_get_formated_percentage_common($row["final_sip_per"])."%</td>";
                        echo "<td class='txtalignright'>".HLP_get_formated_amount_common($row["final_sip"])."</td>";
                        echo "<td>".$row["joining_date_for_increment_purposes"]."</td>";
                        ...
                        {
                            echo "<a href='".site_url("manager/view-employee-sip-dtls/".$rule_id."/".$row["user_id"])."'>View ".$link_txt_name." </a> ";
                        }
                        else
                        {
                            echo "<a href='".site_url("view-employee-sip-dtls/".$rule_id."/".$row["user_id"])."'>View ".$link_txt_name." </a> ";
                        }
                        ...
                        if(count($staff_list) == ($i+1) and count($pk_ids)>0)
                        {
							...
							?>
                          <tr>
                            <td colspan='13' style='text-align:right'><form id="<?php echo $i; ?>" action="<?php echo site_url("manager/dashboard/reject_emp_sip_increment"); ?>" method="post" class="form-inline">
                                <?php echo HLP_get_crsf_field();?>
                                <input type="button" id="btn_reject_<?php echo $i; ?>" onclick="show_remark_dv('<?php echo $i; ?>');" class="btn btn-twitter m-b-sm add-btn" value="Reject"/>
                                <div class="row" id="dv_remark_<?php echo $i; ?>" style="display: none;">
                                  <div class="col-sm-12">
      ...
      <?php if($is_enable_approve_btn == 1 and $this->session->userdata('is_manager_ses') == 1 and isset($is_open_frm_manager_side)){?>
            <a class="anchor_cstm_popup_cls_submit_nxtlevl_<?php echo $rule_id; ?>" href="<?php echo site_url("manager/send-sip-for-next-level/".$rule_id); ?>" onclick="return  request_custom_anchor_confirm('anchor_cstm_popup_cls_submit_nxtlevl_<?php echo $rule_id; ?>','Are you sure, You want?')">
