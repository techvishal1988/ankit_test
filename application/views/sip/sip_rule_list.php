    ...
    <div class="col-sm-8">
     ...
     <div class="col-sm-4">
        ...
        <div class="pull-right">
            <a href="<?php echo site_url("sip-rule-filters/".$performance_cycle_id); ?>"><button class="btn btn-success" type="button">Create New <?php echo $rule_txt_name; ?> Rules</button></a>
        </div>
               <?php } ?>
     </div>
    </div>
...
<div class="col-md-12 background-cls">
   ...
   <?php if($sip_rule_list){
       ...
       ?>
   		<!--<p>sip Rule List Created By Self</p>-->
                <p>Total employees not covered in under this review cycle <a href="<?php echo base_url('sip/employee-not-in-any-rule/'.$performance_cycle_id) ?>"><strong><?php echo $emp_not_in_rule["total_emp_cnt"]; ?></strong></a></p>
		...
		echo "<td>".$row["sip_rule_name"]."</td>";
				...
				if($row["createdby"] == $this->session->userdata("userid_ses"))
				{
					...
					{
						//echo "<a href='".site_url("create-sip-rule/$id")."'>Edit Rule</a>";
						echo "<a data-toggle='tooltip' data-original-title='Edit Rule' data-placement='bottom' href='".site_url("sip-rule-filters/$performance_cycle_id/$id")."'alt='Edit Rule' title='Edit Rule'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a>";//

						 /*| <a data-toggle='tooltip' data-original-title='Delete' data-placement='bottom' onclick="return confirm('<?php echo CV_CONFIRMATION_DELETE_MESSAGE?>')" href="<?php echo site_url("delete-sip-rule/$id"); ?>"alt="Delete" title="Delete"><i class='fa fa-trash' aria-hidden='true'></i></a>*/

					}
					elseif($row["status"] == 3)
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-sip-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='Edit Rule' data-placement='bottom' href='".site_url("sip-rule-filters/$performance_cycle_id/$id")."'alt='Edit Rule' title='Edit Rule'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='Send for Approval' data-placement='bottom' href='".site_url("view-sip-rule-details/$id")."'alt='Send for Approval' title='Send for Approval''><i class='fa fa-thumbs-o-up' aria-hidden='true'></i></a>";
					...
					elseif($row["status"] == 4)
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-sip-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a> | <a data-toggle='tooltip' data-original-title='View Increment List' data-placement='bottom' href='".site_url("view-sip-increments/$id")."'alt='View Increment List' title='View Increment List'><i class='fa fa-list' aria-hidden='true'></i></a>";
					}
					elseif($row["status"] == 5)
					{
						echo "<a data-toggle='tooltip' data-original-title='Edit Rule' data-placement='bottom' href='".site_url("sip-rule-filters/$performance_cycle_id/$id")."'alt='Edit Rule' title='Edit Rule'><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a>";
					...
					{
						echo "<a data-toggle='tooltip' data-original-title='View Rule' data-placement='bottom' href='".site_url("view-sip-rule-details/$id")."'alt='View Rule' title='View Rule'><i class='fa fa-eye' aria-hidden='true'></i></a>";
					}

					if($row["status"] < CV_STATUS_RULE_RELEASED)
					{?>
						 | <a class="anchor_cstm_popup_cls_sip_delete<?php echo $id; ?>"  data-toggle='tooltip' data-original-title='Delete' data-placement='bottom' onclick="return  request_custom_anchor_confirm('anchor_cstm_popup_cls_sip_delete<?php echo $id; ?>','<?php echo $this->lang->line('msg_critical_delete_record'); ?>')" href="<?php echo site_url("delete-sip-rule/$id"); ?>"alt="Delete" title="Delete"><i class='fa fa-trash' aria-hidden='true'></i></a>
					<?php
					}
				}
				else
				...
				if($row["status"] >= 3)
				{
					echo ' | <a target="_blank" data-toggle="tooltip" data-original-title="Print rule" data-placement="bottom" href="'.base_url("printpreview-sip/".$id).'"><i class="fa fa-print" aria-hidden="true"></i></a>';
				}
       ...
       <?php if($approvel_request_list){
        ...
        <p>Pending Approval List of SIP Rules</p>
         ...
         <table id="example" class="table border" style="width: 100%; cellspacing: 0;">
            ...
            <tbody id="tbl_body">
             ...
             foreach($approvel_request_list as $row)
             {
                ...
                if($row["type"]==6)
                {
                    echo "<a href='".site_url("view-sip-rule-details/$rule_id")."'>Approve SIP Rule</a>";
                }
                echo "</td>";
                echo "</tr>";
                $i++;
            ...
            <tbody id="tbl_body">
             ...
             foreach($rules_release_approvel_list as $row)
             {
				if($row["total_emp_in_rule"]==$row["total_appoved_emp_in_rule"])
                {
					...
					<form onsubmit="return request_confirm()" method="post" action="">
                    	<?php echo HLP_get_crsf_field();?>
...
function openEmailTemplatePopup(ruleId) {
 ...
 var csrf_val = $('input[name=csrf_test_name]');
        $.ajax({
            url: '<?Php echo site_url('performance_cycle/getEmailSipRuleTemplate'); ?>',
            type: 'get',
            ...
            success: function (res) {
              ...
              {
                  ancoreTag='<a class="btn btn-primary" onclick="loaderActivate()" href="<?=site_url("sentEmailSipToApprove/")?>'+ruleId+'">Send Mail</a>';
              ...
              {
                  ancoreTag='<a class="btn btn-primary" onclick="loaderActivate()" href="<?=site_url("reminderEmailSipToApprovers/")?>'+ruleId+'">Send Reminder Mail</a>';
